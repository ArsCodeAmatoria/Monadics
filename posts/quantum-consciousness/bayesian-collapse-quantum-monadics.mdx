...
<ContentCard title="Quantum State Evolution Pipeline">
```
|0⟩ Initial State
       ↓
   H Gate (Hadamard)
...
|ψ⟩ = α|0⟩ + β|1⟩ (Superposition)
   ...
   Measurement/Collapse
       ↓
|0⟩ or |1⟩ (Classical Result)
```

The quantum state transitions from a definite initial state through superposition to classical measurement outcome.
</ContentCard>
...
<ContentCard title="Mid-Circuit Measurement and Conditional Logic">
```
|00⟩
  ↓
H(0) → |ψ⟩ = 1/√2(|00⟩ + |10⟩)
...
Mid-circuit measurement enables conditional quantum operations based on measurement outcomes.
</ContentCard>
...
<ContentCard title="From Quantum Amplitudes to Informed Collapse">
```
Quantum State: |ψ⟩ = α|0⟩ + β|1⟩
                    ↓
Extract Amplitudes: |α|², |β|²
                    ↓
Apply Prior: P(0), P(1) ← Prior Knowledge P(ω)
                    ↓
Compute Posterior: P(0|D), P(1|D) ← Likelihood P(D|ω)
                    ↓
Sample from Posterior
                    ↓
Collapsed State: |0⟩ or |1⟩
```

The Bayesian collapse process transforms quantum probabilities into informed decisions through prior knowledge integration.
</ContentCard>
...
<ContentCard title="Functional Composition of Bayesian Collapse">
```
QState a (Superposed)
        ↓
collapseBayesian ← (a -> Double) Prior Function
        ↓
Apply Prior P(a)
        ↓
Compute Posterior P(a|D)
        ↓
Sample a
        ↓
f(a) ← (a -> QState b) Continuation
        ↓
QState b (Result)
```

The monadic structure enables compositional Bayesian collapse through typed functional programming.
</ContentCard>
...
<ContentCard title="Feedback Loop of Conscious Computation">
```
    Quantum Superposition
            ↓
    Bayesian Collapse ← Prior Knowledge
            ↓              ↑
    Classical Decision     │
            ↓              │
    Update Priors ← Observation History
            ↓              ↑
    Next Computation       │
            ↓              │
    New Superposition      │
...
The feedback loop between quantum superposition and classical decision-making creates a conscious-like computational architecture.
</ContentCard>
